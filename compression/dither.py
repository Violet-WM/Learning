...
def apply_dithering(image):
    ...
    for y in range(height):
        for x in range(width):
            ...
            if x < width - 1:
                image.putpixel(
                    (x + 1, y), image.getpixel((x + 1, y)) + int(error * 7 / 16))
            if x > 0 and y < height - 1:
                image.putpixel(
                    (x - 1, y + 1), image.getpixel((x - 1, y + 1)) + int(error * 3 / 16))
            if y < height - 1:
                image.putpixel(
                    (x, y + 1), image.getpixel((x, y + 1)) + int(error * 5 / 16))
            if x < width - 1 and y < height - 1:
                image.putpixel(
                    (x + 1, y + 1), image.getpixel((x + 1, y + 1)) + int(error * 1 / 16))
...
if __name__ == "__main__":
    ...
    image_path = "E:\HAPPY.jpg"  # Replace with your image path
